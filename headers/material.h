...
public:
    explicit metal(const colour &albedo, double fuzz) : albedo(albedo), fuzz(fuzz < 1 ? fuzz : 1) {
    }
...
public:
    emissive(double luminosity, colour albedo): luminosity(luminosity), albedo(albedo) {
    }

    bool scatter(const ray &r, const hit_record &rec, colour &attenuation, ray &scattered) const override {
        attenuation = albedo * luminosity / r.direction().length_squared();
        return false;
    }
